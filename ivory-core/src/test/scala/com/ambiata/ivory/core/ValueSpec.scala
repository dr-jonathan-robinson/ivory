...
class ValueSpec extends Specification with ScalaCheck { def is = s2"""
  ...
  Json int can't lose precision                          $jsonIntDouble
  Json int can't silently overflow                       $jsonIntLong
  Json long can't lose precision                         $jsonLongDouble
"""
  ...
  def jsonIntDouble =
    Value.json.decodeJson(EncodingPrim(IntEncoding), "1.2").toEither must beLeft

  def jsonIntLong =
    Value.json.decodeJson(EncodingPrim(IntEncoding), "50000000000").toEither must beLeft

  def jsonLongDouble =
    Value.json.decodeJson(EncodingPrim(LongEncoding), "5.6").toEither must beLeft
