...
object GenDictionary {
    ...
    Gen.oneOf(fallback, cd.encoding.fold(
      ...
      s => {
        ...
        def sumBy = for {
          ...
          ie <- values.find(v => List(IntEncoding, LongEncoding, DoubleEncoding).contains(v._2.encoding)).map(ie => SumBy(se, ie._1))
        } yield ie

        def countBy = for {
          se <- values.find(v => List(StringEncoding, IntEncoding, LongEncoding).contains(v._2.encoding)).map(_._1)
          ie <- values.find(v => v._1 != se && List(StringEncoding, IntEncoding, LongEncoding).contains(v._2.encoding)).map(ie => CountBySecondary(se, ie._1))
        } yield ie

        Gen.oneOf(countBy, sumBy).flatMap(_.cata(v => Gen.frequency(5 -> Gen.const(v), 5 -> subexpGen), subexpGen))
      },
      _ => fallback
    ))
  }
  ...
  def subExpression(pe: PrimitiveEncoding): Gen[SubExpression] = {
    ...
    pe match {
      case IntEncoding | LongEncoding =>
        Gen.oneOf(Gen.const(CountBy), Gen.const(DaysIn), numeric, all)
      case DoubleEncoding =>
        Gen.oneOf(numeric, all)
      case StringEncoding => Gen.oneOf(
        Gen.oneOf(DaysSinceLatestBy, DaysSinceEarliestBy, CountBy, DaysIn, CountUnique),
