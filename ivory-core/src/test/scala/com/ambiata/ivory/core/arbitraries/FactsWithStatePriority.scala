...
case class FactsWithStatePriority(values: Map[DateTime, PositiveIntSmall]) {
...
object FactsWithStatePriority {
  ...
  implicit def FactsWithStatePriorityArbitrary: Arbitrary[FactsWithStatePriority] =
    Arbitrary(GenPlus.listOfSized(2, 10, arbitrary[(DateTime, PositiveIntSmall)]).map(l =>  FactsWithStatePriority(l.toMap)))
}
