...
trait IvoryLocation {
  ...
  def asHdfsIvoryLocation: RIO[HdfsIvoryLocation] =
    this match {
      case h @ HdfsIvoryLocation(_, _, _) =>
        h.pure[RIO]
      case _ =>
...
case class HdfsIvoryLocation(location: HdfsLocation, configuration: Configuration, codec: Option[CompressionCodec]) extends IvoryLocation {
...
object HdfsIvoryLocation {
  ...
  def apply(location: HdfsLocation, ivory: IvoryConfiguration): HdfsIvoryLocation =
    new HdfsIvoryLocation(location, ivory.configuration, ivory.codec)
...
object IvoryLocation {
  def deleteAll(location: IvoryLocation): RIO[Unit] = location match {
    ...
    case h @ HdfsIvoryLocation(HdfsLocation(path), conf, _) => Hdfs.deleteAll(new Path(path)).run(conf)
  }
  ...
  def delete(location: IvoryLocation): RIO[Unit] = location match {
    ...
    case h @ HdfsIvoryLocation(HdfsLocation(path), conf, _) => Hdfs.delete(new Path(path)).run(conf)
  }
  ...
  def readUtf8(location: IvoryLocation): RIO[String] = location match {
    ...
    case h @ HdfsIvoryLocation(HdfsLocation(path), conf, _) =>
  ...
  def exists(location: IvoryLocation): RIO[Boolean] = location match {
    ...
    case h @ HdfsIvoryLocation(HdfsLocation(path), conf, _) => Hdfs.exists(new Path(path)).run(conf)
  }
  ...
  def writeUtf8(location: IvoryLocation, string: String): RIO[Unit] = location match {
    ...
    case h @ HdfsIvoryLocation(HdfsLocation(path), conf, _) => Hdfs.writeWith(new Path(path), out => Streams.write(out, string)).run(conf)
  }
  ...
  def writeBytes(location: IvoryLocation, bytes: Array[Byte]): RIO[Unit] = location match {
    ...
    case h @ HdfsIvoryLocation(HdfsLocation(path), conf, _) => Hdfs.writeWith(new Path(path), out => Streams.writeBytes(out, bytes)).run(conf)
  }
  ...
  def isDirectory(location: IvoryLocation): RIO[Boolean] = location match {
    ...
    case h @ HdfsIvoryLocation(HdfsLocation(path), conf, _) => Hdfs.isDirectory(new Path(path)).run(conf)
  }
  ...
  def parseUri(s: String, ivory: IvoryConfiguration): String \/ IvoryLocation = try {
    ...
    uri.getScheme match {
      case null =>
        ...
        HdfsIvoryLocation(HdfsLocation(new File(uri.getPath).getAbsolutePath), ivory.configuration, ivory.codec).right
  ...
  def fromLocation(loc: Location, ivory: IvoryConfiguration): IvoryLocation = loc match {
    ...
    case h: HdfsLocation =>
      HdfsIvoryLocation(h, ivory.configuration, ivory.codec)
  }
}
