...
object Value {
  ...
  def foldNumeric[A](value: Value)(int: Int => A, long: Long => A, double: Double => A, unknown: => A): A = value match {
    case IntValue(i)     => int(i)
    case LongValue(l)    => long(l)
    case DoubleValue(d)  => double(d)
    case BooleanValue(_) => unknown
    case StringValue(_)  => unknown
    case DateValue(_)    => unknown
    case TombstoneValue  => unknown
    case StructValue(_)  => unknown
    case ListValue(_)    => unknown
  }
  ...
  object json {
    ...
    def toJson(v: Value): Json = v match {
      ...
      case IntValue(i)      => i.asJson
      case LongValue(l)     => l.asJson
