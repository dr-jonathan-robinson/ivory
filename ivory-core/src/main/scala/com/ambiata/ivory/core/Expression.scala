...
object Expression {
  ...
  def parse(exp: String): String \/ Expression = {
    ...
    def parseSub(sexp: SubExpression, args: List[String]): String \/ Expression = args match {
      case Nil      => BasicExpression(sexp).right
      case h :: Nil => StructExpression(h, sexp).right
      case _ :: xs  => s"Trailing arguments ${xs.mkString(",")} in expression would be ignored".left
    }
    ...
    PartialFunction.condOpt(args) {
      ...
      case "days_in" :: tail                => parseSub(DaysIn, tail)
      ...
      case "latestN" :: num :: tail => parseInt(num).flatMap(n => parseSub(LatestN(n), tail))
  ...
  def validate(exp: Expression, encoding: Encoding): String \/ Unit = {
    ...
    def validateSub(sexp: SubExpression, subenc: PrimitiveEncoding): String \/ Unit = sexp match {
      ...
      case CountBy => subenc match {
        ...
        case _              => "Encoding not supported, only support string, int, long, or double".left
      }
      case DaysIn => subenc match {
        case StringEncoding => ok
        case IntEncoding    => ok
        case LongEncoding   => ok
        case _              => "Encoding not supported, only support string, int, or long".left
      }
      case NumFlips         => ok
    ...
    (exp match {
      ...
      case CountBySecondary(key, field)             => encoding.fold(
        ...
        s => for {
          ...
          f <- s.values.get(field).map(_.encoding).toRightDisjunction(s"Struct field not found '$field'")
          _ <- {
            val valid = List(StringEncoding, IntEncoding, LongEncoding, DoubleEncoding)
            if(valid.contains(k) && valid.contains(f)) ok else "count_by fields are required to be strings, ints, longs, or doubles".left
          }
        } yield (),
        _ => "count_by requires struct encoding".left
      )
  ...
  def expressionEncoding(expression: Expression, source: Encoding): Encoding = {
    def getExpressionEncoding(exp: SubExpression, enc: Encoding): Encoding = exp match {
      ...
      case DaysIn             => StructEncoding(Map()).toEncoding
...
case object DaysIn extends SubExpression
