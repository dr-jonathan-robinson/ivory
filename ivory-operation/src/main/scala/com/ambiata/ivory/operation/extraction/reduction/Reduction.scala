...
object Reduction {
  ...
  def fromExpression(exp: Expression, encoding: Encoding, dates: DateOffsetsLazy): Option[Reduction] = exp match {
    ...
    case CountBySecondary(k, f)       => encoding.fold(
      _ => None,
      s => s.values.get(k).map(_.encoding).tuple(s.values.get(f).map(_.encoding)).flatMap(t => {
        def reductionFold[A, B](rva: ReductionValueFromPrim[A], rvb: ReductionValueFromPrim[B]) =
          Some(new ReductionFold2StructWrapper(k, f, new CountBySecondaryReducer[A, B], rva, rvb, ReductionValueStruct[A, collection.mutable.Set[B]](ReductionValueSetSize())))

        t match {
          case (StringEncoding, StringEncoding) => reductionFold[String, String](ReductionValueString, ReductionValueString)
          case (StringEncoding, IntEncoding)    => reductionFold[String, Int](ReductionValueString, ReductionValueInt)
          case (StringEncoding, LongEncoding)   => reductionFold[String, Long](ReductionValueString, ReductionValueLong)
          case (StringEncoding, DoubleEncoding) => reductionFold[String, Double](ReductionValueString, ReductionValueDouble)
          case (IntEncoding, StringEncoding)    => reductionFold[Int, String](ReductionValueInt, ReductionValueString)
          case (IntEncoding, IntEncoding)       => reductionFold[Int, Int](ReductionValueInt, ReductionValueInt)
          case (IntEncoding, LongEncoding)      => reductionFold[Int, Long](ReductionValueInt, ReductionValueLong)
          case (IntEncoding, DoubleEncoding)    => reductionFold[Int, Double](ReductionValueInt, ReductionValueDouble)
          case (LongEncoding, StringEncoding)   => reductionFold[Long, String](ReductionValueLong, ReductionValueString)
          case (LongEncoding, IntEncoding)      => reductionFold[Long, Int](ReductionValueLong, ReductionValueInt)
          case (LongEncoding, LongEncoding)     => reductionFold[Long, Long](ReductionValueLong, ReductionValueLong)
          case (LongEncoding, DoubleEncoding)   => reductionFold[Long, Double](ReductionValueLong, ReductionValueDouble)
          case (DoubleEncoding, StringEncoding) => reductionFold[Double, String](ReductionValueDouble, ReductionValueString)
          case (DoubleEncoding, IntEncoding)    => reductionFold[Double, Int](ReductionValueDouble, ReductionValueInt)
          case (DoubleEncoding, LongEncoding)   => reductionFold[Double, Long](ReductionValueDouble, ReductionValueLong)
          case (DoubleEncoding, DoubleEncoding) => reductionFold[Double, Double](ReductionValueDouble, ReductionValueDouble)
          case _                                => None
        }
      }),
      _ => None
  ...
  def fromSubExpression(exp: SubExpression, encoding: PrimitiveEncoding, dates: DateOffsetsLazy, f: EncodedReduction): Option[Reduction] = exp match {
    ...
    case CountBy => condOpt(encoding) {
      ...
      case LongEncoding    => f.l(new CountByReducer, ReductionValueStruct[Long, Long](ReductionValueLong))
      case DoubleEncoding  => f.d(new CountByReducer, ReductionValueStruct[Double, Long](ReductionValueLong))
    }
    case DaysIn => condOpt(encoding) {
      case StringEncoding  => f.s(new DaysInReducer(dates.dates, ""), ReductionValueStruct[String, Int](ReductionValueInt))
      case IntEncoding     => f.i(new DaysInReducer(dates.dates, 0),  ReductionValueStruct[Int, Int](ReductionValueInt))
      case LongEncoding    => f.l(new DaysInReducer(dates.dates, 0),  ReductionValueStruct[Long, Int](ReductionValueInt))
    }
