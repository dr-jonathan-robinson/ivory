...
import scalaz.{Reducer => _, Value => _, _}, Scalaz._
...
object FactStatsJob {
  def run(repository: HdfsRepository, dictionary: Dictionary, datasets: Datasets, target: HdfsIvoryLocation): RIO[Unit] = for {
    ...
    stats <- RIO.safe {
      ...
      MultipleOutputs.addNamedOutput(job, Keys.NumericalOut, classOf[TextOutputFormat[_, _]],  classOf[NullWritable], classOf[Text])
      MultipleOutputs.addNamedOutput(job, Keys.CategoricalOut, classOf[TextOutputFormat[_, _]],  classOf[NullWritable], classOf[Text])
    ...
    _ <- Committer.commit(ctx, {
      case "numerical"   => (target </> DirPath.unsafe("numerical")).toHdfsPath
      case "categorical" => (target </> DirPath.unsafe("categorical")).toHdfsPath
    }, true).run(repository.configuration)
  } yield stats

  object Keys {
    val NumericalOut = "numerical"
    val CategoricalOut = "categorical"
...
abstract class FactStatsMapper[K <: Writable] extends CombinableMapper[K, BytesWritable, BytesWritable, BytesWritable] with MrFactFormat[K, BytesWritable] {
  ...
  override def map(key: K, value: BytesWritable, context: MapperContext): Unit = {
    ...
    if(featureId.isEmpty)
      ...
      FactStatsMapper.emitStats(fact, numericalStats, categoricalStats, emitter, kout, vout, featureId.get, serialiser)
  }
}
...
object FactStatsMapper {
  ...
  def emitStats(fact: Fact, numericalStats: ThriftNumericalStats, categoricalStats: ThriftCategoricalStats, emitter: Emitter[BytesWritable, BytesWritable],
                kout: BytesWritable, vout: BytesWritable, featureId: FeatureIdIndex, serialiser: ThriftSerialiser): Unit = {
    ...
    Value.foldNumeric(fact.value)(emitNumerical[Int], emitNumerical[Long], emitNumerical[Double], ())
  }
}
...
class FactStatsReducer extends Reducer[BytesWritable, BytesWritable, NullWritable, Text] {
  ...
  /** Class to emit the key/value bytes, created once per mapper */
  var numericalEmitter: MrOutputEmitter[NullWritable, Text] = null
  var categoricalEmitter: MrOutputEmitter[NullWritable, Text] = null
  ...
  override def setup(context: ReducerContext): Unit = {
    ...
    numericalEmitter = MrOutputEmitter(FactStatsJob.Keys.NumericalOut, out, "numerical/part")
    categoricalEmitter = MrOutputEmitter(FactStatsJob.Keys.CategoricalOut, out, "categorical/part")
  }
  ...
  override def reduce(key: BytesWritable, iter: JIterable[BytesWritable], context: ReducerContext): Unit = {
    ...
    FactStatsReducer.reduce(ty, featureIds(feature), date, iter.iterator, numericalStats, categoricalStats, numericalEmitter, categoricalEmitter, vout, serialiser)
    ()
  }
}
...
object FactStatsReducer {
  ...
  def reduce(ty: FactStatsWritable.StatsType, featureId: FeatureId, date: Date, iter: JIterator[BytesWritable], numericalStats: ThriftNumericalStats,
             categoricalStats: ThriftCategoricalStats, numericalEmitter: Emitter[NullWritable, Text], categoricalEmitter: Emitter[NullWritable, Text],
             vout: Text, serialiser: ThriftSerialiser): Unit = {

    ty match {
      case FactStatsWritable.Numerical =>
        FactStatsCombiner.combineNumerical(iter, numericalStats, serialiser)
        numerical(featureId, date, numericalStats).foreach(n => {
          vout.set(n.asJson.nospaces)
          numericalEmitter.emit(NullWritable.get, vout)
        })
      case FactStatsWritable.Categorical =>
        FactStatsCombiner.combineCategorical(iter, categoricalStats, serialiser)
        categorical(featureId, date, categoricalStats).foreach(c => {
          vout.set(c.asJson.nospaces)
          categoricalEmitter.emit(NullWritable.get, vout)
        })
    }
  }

  def numerical(featureId: FeatureId, date: Date, numericalStats: ThriftNumericalStats): Option[NumericalFactStatistics] =
    (numericalStats.getCount > 0).option({
