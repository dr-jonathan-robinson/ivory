...
object SquashJob {
   ...
   */
  def squash(repository: Repository, dictionary: Dictionary, lookup: SquashReducerLookup, conf: SquashConfig,
  ...
  def run(job: Job, ctx: MrContext, reducers: Int, dict: Dictionary, reducerLookup: SquashReducerLookup, output: Path,
    ...
    val (featureIdLookup, reductionLookup, reducerLookupThrift) = dictToLookup(dictionary, latest, reducerLookup)
    ctx.thriftCache.push(job, SnapshotJob.Keys.FeatureIdLookup, featureIdLookup)
    ctx.thriftCache.push(job, ReducerLookups.Keys.ReducerLookup, reducerLookupThrift)
  ...
  def dictToLookup(dictionary: DictionaryConcrete, latest: Boolean, lookup: SquashReducerLookup):
    (FeatureIdLookup, FeatureReductionLookup, ReducerLookup) = {
    ...
    dictionary.sources.foreach { case (fid, cg) =>
      dictionary.byFeatureIndexReverse.get(fid).foreach { i =>
        featureIdLookup.putToIds(fid.toString, i)
        reductionLookup.putToReductions(i, concreteGroupToReductions(fid, cg, latest).asJava)
        reducerLookup.putToReducers(i, lookup.lookup.get(fid).cata(_.toInt, 1))
      }
    }
    (featureIdLookup, reductionLookup, reducerLookup)
  }
