...
object GroupByEntityOutput {
  ...
  def createWithDictionary(repository: Repository, input: ShadowOutputDataset, output: ShadowOutputDataset, dictionary: Dictionary,
                           format: GroupByEntityFormat, numReducers: Option[Int]): RIO[Unit] = for {
    hdfsRepo       <- repository.asHdfsRepository
    in             =  input.hdfsPath
    out            =  output.hdfsPath
    reducers       <- numReducers.cata(RIO.ok, ReducerSize.calculate(in, 256.mb).run(hdfsRepo.configuration))
