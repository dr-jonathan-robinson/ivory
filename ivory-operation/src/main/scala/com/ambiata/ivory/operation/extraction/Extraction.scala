...
object Extraction {
  ...
  def extract(formats: OutputFormats, input: ShadowOutputDataset, dictionary: Dictionary,
              cluster: Cluster, conf: ExtractionConfig): RepositoryTIO[Unit] = RepositoryT.fromRIO(repository =>
    formats.outputs.traverseU { case (format, output) => for {
      ...
      _ <- format match {
        case OutputFormat(Form.Sparse, FileFormat.Thrift) =>
          GroupByEntityOutput.createWithDictionary(repository, input, s, dictionary, GroupByEntityFormat.SparseThrift, conf.groupByEntityReducers)
        case OutputFormat(Form.Dense, FileFormat.Thrift) =>
          GroupByEntityOutput.createWithDictionary(repository, input, s, dictionary, GroupByEntityFormat.DenseThrift, conf.groupByEntityReducers)
        ...
        case OutputFormat(Form.Dense, FileFormat.Text(delimiter, escaping, format)) =>
          GroupByEntityOutput.createWithDictionary(repository, input, s, dictionary,
            GroupByEntityFormat.DenseText(delimiter, formats.missingValue, escaping, format), conf.groupByEntityReducers)
      }
