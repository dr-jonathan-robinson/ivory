...
case class SquashReducerLookup(lookup: Map[FeatureId, FeatureReducerOffset])

object SquashReducerLookup {

  def createFromChord(chord: ChordPlan, reducers: Int): SquashReducerLookup =
  ...
  def createFromSnapshot(snapshot: Snapshot, dictionary: Dictionary, reducers: Int): SquashReducerLookup =
  ...
  def createLookup(dictionary: Dictionary, reducers: Map[FeatureId, Int]): SquashReducerLookup =
    // Create a sub-index for all of the features so they don't overlap on the reducers
    SquashReducerLookup(reducers.zipWithIndex.map {
      case ((fid, count), i) =>
