...
"""
  ...
  def lookup = prop((d: VirtualDictionaryWindow, d2: DictionaryWithoutKeyedSet, s: NaturalInt, e: Int) => s.value != Short.MaxValue ==> {
    ...
    val lookupV = create.lookup.get(d.vdict.vd.source)
    val windowReducers = create.lookup.values.map(_.count.toInt).sum
    windowReducers must beGreaterThan(reducers - d2.value.byConcrete.sources.size) and
      (FeatureReducerOffset.getReducer(lookupV.map(_.toInt).getOrElse(-1), e & Int.MaxValue) must beGreaterThanOrEqualTo(0))
  })
