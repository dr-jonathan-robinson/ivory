...
object ReducerUtil {
  ...
  def runWithDatesAndTombstones[A, B, C](r: ReductionFoldWithDate[A, B, C], l: List[(Option[B], Date)]): C =
    r.aggregate(l.foldLeft(r.initial) {
      case (a, (Some(v), d)) => r.foldWithDate(a, v, d)
      case (a, (None,    d)) => r.tombstoneWithDate(a, d)
    })
