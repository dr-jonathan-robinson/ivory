...
import com.ambiata.notion.core.{KeyName, SequenceFileIO, LocationIO}
...
"""
  ...
  def run[A <: ThriftLike](format: CatThriftFormat, thrifts: List[A]): RIO[List[String]] = {
    RepositoryBuilder.using { repo => for {
      ...
      _ <- SequenceFileIO.writeValues(i.location, LocationIO(repo.configuration, Clients.s3), thrifts)(t =>
        new BytesWritable(ThriftSerialiser().toBytes(t)))
