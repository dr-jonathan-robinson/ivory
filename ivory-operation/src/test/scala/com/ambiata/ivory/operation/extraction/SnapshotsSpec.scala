...
"""
  ...
  def run(dictionary: Dictionary, facts: List[List[Fact]], d: Date): RIO[MatchResult[List[Fact]]] = for {
    r <- RepositoryBuilder.repository
    _ <- RepositoryBuilder.createRepo(r, dictionary, facts)
    s <- Snapshots.takeSnapshot(r, IvoryFlags.default, d)
    o <- RepositoryBuilder.readSnapshot(r, s.id)
