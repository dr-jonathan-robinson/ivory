...
object build extends Build {
  ...
  , settings = standardSettings ++ lib("storage") ++ Seq[Settings](
      name := "ivory-storage"
                                               ...
                                               depend.argonaut ++ depend.poacher(version.value) ++ depend.hadoop(version.value) ++ depend.specs2 ++
  ...
  lazy val testing = Project(
    ...
    , settings = standardSettings ++ lib("testing") ++ Seq[Settings](
      name := "ivory-testing"
    ) ++ Seq[Settings](libraryDependencies ++= depend.scalaz ++ depend.mundane ++ depend.poacher(version.value) ++ depend.specs2 ++ depend.saws ++ depend.notion(version.value) ++ depend.hadoop(version.value))
